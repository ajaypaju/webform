...
- **Submissions in the dashboard**: the list and CSV export are API-only today; the dashboard has login and the builder.
...
- **The builder is exercised over HTTP, not in a browser.** Its endpoints, escaping and error attribution are tested; the editor's DOM behaviour (move/remove, visibility editor, preview) was checked by hand only.
